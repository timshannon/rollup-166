  ...
  if (mods) {
    ...
    if (mods.indexOf('u') >= 0 && !regexpUnicodeSupport) {
      ...
      tmp = tmp.replace(/\\u\{([0-9a-fA-F]+)\}/g, function (match, code, offset) {
        code = Number("0x" + code);
        if (code > 0x10FFFF) _this5.raise(start + offset + 3, "Code point out of bounds");
        return "x";
      });
...
function _parse(input, options) {
  return new Parser(options, input).parse();
}

function walk(ast, _ref2) {
...
var Scope = (function () {
    ...
    if (options.params) {
      options.params.forEach(function (param) {
        extractNames(param).forEach(function (name) {
          _this6.declarations[name] = new Declaration(name);
        });
      });
    }
  }
  ...
  Scope.prototype.addDeclaration = function addDeclaration(node, isBlockDeclaration, isVar) {
    ...
    if (!isBlockDeclaration && this.isBlockScope) {
      ...
      extractNames(node.id).forEach(function (name) {
        _this7.declarations[name] = new Declaration(name);
      });
    }
  };
  ...
  Scope.prototype.eachDeclaration = function eachDeclaration(fn) {
    ...
    keys(this.declarations).forEach(function (key) {
      fn(key, _this8.declarations[key]);
    });
  };
...
var Statement = (function () {
  ...
  Statement.prototype.analyse = function analyse() {
    ...
    walk(this.node, {
      enter: function (node, parent) {
        var isStringLiteral = node.type === 'TemplateElement' || node.type === 'Literal' && typeof node.value === 'string';
...
var SyntheticDefaultDeclaration = (function () {
  function SyntheticDefaultDeclaration(node, statement, name) {
    _classCallCheck(this, SyntheticDefaultDeclaration);
...
var SyntheticNamespaceDeclaration = (function () {
  function SyntheticNamespaceDeclaration(module) {
    ...
    _classCallCheck(this, SyntheticNamespaceDeclaration);
    ...
    module.getExports().forEach(function (name) {
      _this10.originals[name] = module.traceExport(name);
    });
  }
  ...
  SyntheticNamespaceDeclaration.prototype.renderBlock = function renderBlock(indentString) {
    ...
    var members = keys(this.originals).map(function (name) {
      var original = _this11.originals[name];
...
var Module = (function () {
    ...
    if (source) {
      ...
      if (node.type === 'ExportAllDeclaration') {
        ...
        node.specifiers.forEach(function (specifier) {
          _this13.reexports[specifier.exported.name] = {
            source: source,
      ...
      // export function foo () {}
          ...
          if (node.specifiers.length) {
            ...
            node.specifiers.forEach(function (specifier) {
              ...
              _this13.exports[exportedName] = { localName: localName };
            });
          } else {
            ...
            this.exports[_name2] = { localName: _name2 };
          }
        }
  };
  ...
  Module.prototype.addImport = function addImport(statement) {
    ...
    node.specifiers.forEach(function (specifier) {
      ...
      _this14.imports[localName] = { source: source, name: name, module: null };
    });
  };
  ...
  Module.prototype.analyse = function analyse() {
    ...
    this.statements.forEach(function (statement) {
      ...
      statement.scope.eachDeclaration(function (name, declaration) {
        _this15.declarations[name] = declaration;
      });
    });
  };

  Module.prototype.basename = function basename() {
    return makeLegalIdentifier(_basename(this.id).slice(0, -extname(this.id).length));
  };
  ...
  Module.prototype.bindAliases = function bindAliases() {
    ...
    keys(this.declarations).forEach(function (name) {
      var declaration = _this16.declarations[name];
      ...
      statement.references.forEach(function (reference) {
        ...
        var otherDeclaration = _this16.trace(reference.name);
  ...
  Module.prototype.bindImportSpecifiers = function bindImportSpecifiers() {
    ...
    [this.imports, this.reexports].forEach(function (specifiers) {
      keys(specifiers).forEach(function (name) {
        ...
        var id = _this17.resolvedIds[specifier.source];
        specifier.module = _this17.bundle.moduleById[id];
      });
    });

    this.exportAllModules = this.exportAllSources.map(function (source) {
      var id = _this17.resolvedIds[source];
      return _this17.bundle.moduleById[id];
    });
  };
  ...
  Module.prototype.bindReferences = function bindReferences() {
    ...
    this.statements.forEach(function (statement) {
      ...
      if (statement.node.type === 'ExportNamedDeclaration' && statement.node.specifiers.length) {
        ...
        if (_this18 !== _this18.bundle.entryModule) return;
      }

      statement.references.forEach(function (reference) {
        var declaration = reference.scope.findDeclaration(reference.name) || _this18.trace(reference.name);

        if (declaration) {
          declaration.addReference(reference);
        } else {
          // TODO handle globals
          _this18.bundle.assumedGlobals[reference.name] = true;
        }
      });
    });
  };
  ...
  Module.prototype.consolidateDependencies = function consolidateDependencies() {
    ...
    this.dependencies.forEach(function (source) {
      var id = _this19.resolvedIds[source];
      var dependency = _this19.bundle.moduleById[id];
    ...
    this.statements.forEach(function (statement) {
      statement.references.forEach(function (reference) {
        ...
        if (declaration && declaration.statement) {
          var _module2 = declaration.statement.module;
          if (_module2 === _this19) return;

          // TODO disregard function declarations
          if (reference.isImmediatelyUsed) {
            strongDependencies[_module2.id] = _module2;
          }
        }
      });
    });
  ...
  Module.prototype.parse = function parse(ast) {
    ...
    if (!ast) {
      ...
      try {
        ast = _parse(this.source, {
          ecmaVersion: 6,
          sourceType: 'module',
          onComment: function (block, text, start, end) {
            return _this20.comments.push({ block: block, text: text, start: start, end: end });
          },
          preserveParens: true
        });
      } catch (err) {
    ...
    walk(ast, {
      enter: function (node) {
        _this20.magicString.addSourcemapLocation(node.start);
        _this20.magicString.addSourcemapLocation(node.end);
      }
    });
    ...
    ast.body.forEach(function (node) {
      ...
      if (node.type === 'VariableDeclaration' && node.declarations.length > 1) {
        ...
        if (!lastStatement || !lastStatement.node.isSynthetic) {
          _this20.magicString.remove(node.start, node.declarations[0].start);
        }
        ...
        node.declarations.forEach(function (declarator) {
          ...
          var statement = new Statement(syntheticNode, _this20, start, end);
        ...
        lastChar = node.end; // TODO account for trailing line comment
          ...
          do {
            comment = _this20.comments[commentIndex];
            if (!comment) break;
          ...
          var statement = new Statement(node, _this20, start, end);
          statements.push(statement);
  ...
  Module.prototype.render = function render(es6) {
    ...
    this.statements.forEach(function (statement) {
      ...
      if (statement.node.isSynthetic) {
        ...
        var declaration = _this21.declarations[statement.node.declarations[0].id.name];
      ...
      statement.references.forEach(function (reference) {
        ...
        if (declaration) {
          ...
          if (reference.isShorthandProperty) {
            magicString.insert(end, ': ' + _name3);
          } else {
            magicString.overwrite(start, end, _name3, true);
          }
        }
      });
      ...
      if (statement.isExportDeclaration) {
        ...
        if (statement.node.type === 'ExportNamedDeclaration' && statement.node.declaration.type === 'VariableDeclaration') {
          var _name4 = statement.node.declaration.declarations[0].id.name;
          var declaration = _this21.declarations[_name4];
        ...
        // TODO default exports need different treatment
            ...
            var defaultDeclaration = _this21.declarations.default;
  ...
  Module.prototype.traceExport = function traceExport(name, importer) {
    ...
    for (var i = 0; i < this.exportAllModules.length; i += 1) {
      var _module3 = this.exportAllModules[i];
      var declaration = _module3.traceExport(name, this);
...
function cjs(bundle, magicString, _ref5, options) {
  var exportMode = _ref5.exportMode;
...
function iife(bundle, magicString, _ref6, options) {
  var exportMode = _ref6.exportMode;
  var indentString = _ref6.indentString;
...
function umd(bundle, magicString, _ref7, options) {
  var exportMode = _ref7.exportMode;
  var indentString = _ref7.indentString;
...
function defaultLoader(id, options) {
  ...
  var source = readFileSync(id, 'utf-8');
...
var Bundle = (function () {
  function Bundle(options) {
    ...
    ['module', 'exports'].forEach(function (global) {
      return _this22.assumedGlobals[global] = true;
    });
  }
  ...
  Bundle.prototype.fetchModule = function fetchModule(id) {
    ...
    return Promise$1.resolve(this.load(id, this.loadOptions)).then(function (source) {
      ...
      var module = new Module({ id: id, source: source, ast: ast, bundle: _this24 });
  ...
  Bundle.prototype.fetchAllDependencies = function fetchAllDependencies(module) {
    ...
    var promises = module.dependencies.map(function (source) {
      return Promise$1.resolve(_this25.resolveId(source, module.id, _this25.resolveOptions)).then(function (resolvedId) {
        module.resolvedIds[source] = resolvedId || source;

        // external module
        if (!resolvedId) {
          if (!_this25.moduleById[source]) {
            var _module4 = new ExternalModule(source);
